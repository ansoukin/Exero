-- seed_courses_data.sql - 示例学期与课表数据（SPEC V2 11.2 节：演示模式数据源）
--
-- ⚠️ 本文件不是 refinery 迁移文件，不会被自动执行。
--    仅作为 load_demo_data 命令的数据源（通过 include_str! 编译期嵌入）。
--
-- 背景：原本 V004__seed_courses.sql 在 refinery 迁移时自动执行，
--       导致清库重建后示例数据被自动插入，与"跳过向导应得空课表"冲突（Beta6 修复）。
--       现在迁移文件 V004 保持空注释状态，示例数据移到本文件，
--       仅当用户主动点击"加载示例数据"时通过 load_demo_data 命令执行。
--
-- 示例数据内容：
