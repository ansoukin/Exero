...
检查：
1. 扩展包 `exero_api_version` 在新版本兼容区间内（Beta6 兼容所有 `0.4.x`）
2. Manifest 结构没变（Beta5 重构过一次扩展机制，`pack_type` 字段有变化）
3. 插件重新编译 .dll（Beta6 Rust MSRV 没升，但 linker 改动可能导致旧 dll 加载失败）

### 如何知道当前文档适用哪个 Exero 版本？

本套文档每页顶部都有 `适用版本：V0.4.0-Beta6` 徽章。前后端每次 Beta 变更时需同步更新徽章版本号和对应文档内容。
...
Beta6 不支持（硬编码 `10_000` ms）。需要更长执行时间请写 Rust 动作。
...
间接可以（.dll 端自己开 rusqlite 连接读 SQLite 文件路径）。但 Beta6 不提供"官方数据库访问 API"——这是有意为之的隔离边界。如果确实需要：
...
| 对象 | 推荐阅读 |
|---|---|
| 扩展包开发者（写插件/动作包） | **本套 VitePress 文档**（面向开发，完整示例 + API 参考） |
| 改 Exero 核心代码 | 先读 [SPEC 摘要](/spec-summary)（5 分钟关键决策），细节查 SPEC.md |
| AI 协作助手 | 先读 SPEC 摘要 + [架构概览](/architecture)（减少 token 消耗），缺细节再读 SPEC.md 具体章节 |
